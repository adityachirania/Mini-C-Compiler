...
void print_symbol_table(struct symbol_table_entry **table)
{
    ...
    while (p != NULL)
    {
        ...
        {
            if(strcmp(p->class, "IDENTIFIER") == 0)
            {
...
int append_pointer_type(struct symbol_table_entry **table, char *symbol)
{
    struct symbol_table_entry *p = *table;
    while (p != NULL)
    {
...
int insert_symbol_nesting(struct symbol_table_entry **table, char *symbol, int nesting)
{
    // return;
    struct symbol_table_entry *p = *table;
    while (p != NULL)
